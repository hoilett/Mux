...
//Mux(int S0)
//This method is a constructor for a 1-bit (2-channel) multiplexer. This
//constructor assumes the "Enable" pin is tied to ground placing the mux in an
//"always on" state. In this state, the mux effectively acts like a SPDT switch.
Mux::Mux(int S0):S0(S0), ctrlPins(1)
{
	pins[0] = S0;
...
//Mux(int S0, int E)
//This method is a constructor for a 1-bit (2-channel) multiplexer.
Mux::Mux(int S0, int E):S0(S0), E(E), ctrlPins(2)
{
	pins[0] = S0;
	pins[1] = E;
...
Mux::Mux(int S0, int S1, int E):S0(S0), S1(S1), E(E), ctrlPins(3)
{
	pins[0] = S0;
	pins[1] = S1;
	pins[2] = E;
...
Mux::Mux(int S0, int S1, int S2, int E):S0(S0), S1(S1), S2(S2), E(E), ctrlPins(4)
{
	pins[0] = S0;
	pins[1] = S1;
	pins[2] = S2;
...
Mux::Mux(int S0, int S1, int S2, int S3, int E)
	:S0(S0), S1(S1), S2(S2), S3(S3), E(E), ctrlPins(5)
{
	pins[0] = S0;
	pins[1] = S1;
	pins[2] = S2;
...
Mux::Mux(int S0, int S1, int S2, int S3, int S4, int E)
	:S0(S0), S1(S1), S2(S2), S3(S3), S4(S4), E(E), ctrlPins(6)
{
	pins[0] = S0;
	pins[1] = S1;
	pins[2] = S2;
...
{
	if(ctrlPins == 1) //1-bit Mux as SPDT switch
	{
		digitalWrite(pins[ctrlPins-1], bitRead(channel, ctrlPins-1));
	}
	else
	{
		disable();

		for(int i = 0; i < ctrlPins-1; ++i)
		{
...
//Cannot be used for a single channel mux
//with the enable pin not tied to a digital I/O.
